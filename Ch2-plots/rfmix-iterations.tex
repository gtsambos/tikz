...
\draw ($(s1) + (r1) + (c1) + (ref) + (-2,0)$) node[anchor=east] {{\bf\texttt{Step 1.}}};
\draw ($(s2) + (r1) + (c1) + (ref) + (-2,0)$) node[anchor=east] {\bf\texttt{Step 2.}};
\draw ($(s3) + (r1) + (c1) + (ref) + (-2,0)$) node[anchor=east] {\bf\texttt{Step 3.}};

\draw ($(s1) + (r1) + (c2) + (ref) + (0,4)$) node[anchor=south] {\texttt{Training chromosomes}};
\draw ($(s1) + (r1) + (c2) + (adm) + (0,4)$) node[anchor=south] {\texttt{Test chromosomes}};
...
\draw[->,bend left=45] (start) to node {{\scriptsize \texttt{inference}}} node [swap] {} (end); 
